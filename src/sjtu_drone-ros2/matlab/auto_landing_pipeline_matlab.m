...
function cfg = defaultConfig()
    ...
    cfg.wind.start_delay_after_hover_sec = 5.0;
    ...
    cfg.thresholds.landed_altitude_max_m = 0.20;
...
function cleanupSimulationProcesses(cfg, launchPid)
    ...
    [~, out] = system(['bash -i -c "pgrep -f \"[r]os2 launch sjtu_drone_bringup sjtu_drone_bringup.launch.py\" | xargs -r kill -9 || true; ' ...
        'pkill -9 gzserver || true; ' ...
        'pkill -9 gzclient || true; ' ...
        'pkill -f robot_state_publisher || true; ' ...
        'pkill -f joint_state_publisher || true; ' ...
        'pkill -f spawn_drone || true; ' ...
        'pkill -x apriltag_state_bridge || true; ' ...
        'pkill -x component_container || true; ' ...
        'pkill -x teleop_joystick || true; ' ...
        'pkill -x teleop || true; ' ...
        'pkill -x joy_node || true; ' ...
        'pkill -x rviz2 || true"']);
    if ~isempty(strtrim(out))
        fprintf('[PIPELINE] Cleanup output:\n%s\n', out);
    else
        fprintf('[PIPELINE] Cleanup complete (pkill/gz kill).\n');
    end
end
            ...
            if cfg.learning.enable && isFlying && (controlPhase == "xy_hold")
                ...
                if randomLandingPlanned && ~landingSent
                    ...
                    elseif tk >= randomLandingEndT
                        ...
                        landingSent = true;
                        landingSentT = tk;
                        fprintf('[PIPELINE] s%03d random landing command sent at t=%.1fs\n', scenarioId, tk);
                    end
                end
            end
