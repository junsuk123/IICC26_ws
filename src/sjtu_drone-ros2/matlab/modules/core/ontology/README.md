...
$$
\text{wind\_risk\enc} = \sigma(w_w^T[\|\mathbf{v}_w\|, \|\mathbf{a}_w\|, \ldots] + b_w) \in [0,1]
$$

$$
\text{alignment\enc} = \sigma(w_a^T[e_{tag}, \text{trend}, \ldots] + b_a) \in [0,1]
$$

$$
\text{visual\enc} = \sigma(w_v^T[\text{stability}, \text{jitter}, \ldots] + b_v) \in [0,1]
$$
