function tbl = autosimSummaryTable(results)
    if isempty(results)
        tbl = table();
        return;
    end

    tbl = struct2table(results);
    wanted = {
        'scenario_id','label','success','failure_reason', ...
        'hover_height_cmd','landing_height_m', ...
        'wind_speed_cmd','wind_dir_cmd', ...
        'mean_wind_speed','max_wind_speed', ...
        'wind_velocity_x','wind_velocity_y','wind_velocity', ...
        'wind_acceleration_x','wind_acceleration_y','wind_acceleration', ...
        'mean_abs_roll_deg','mean_abs_pitch_deg', ...
        'mean_abs_vz','max_abs_vz', ...
        'mean_tag_error','max_tag_error', ...
        'final_altitude','final_abs_speed','final_abs_roll_deg','final_abs_pitch_deg','final_tag_error', ...
        'stability_std_z','stability_std_vz','stability_std_vz_osc','touchdown_accel_rms','contact_count', ...
        'mean_imu_ang_vel','max_imu_ang_vel','mean_imu_lin_acc','max_imu_lin_acc', ...
        'max_contact_force','arm_force_fl_mean','arm_force_fr_mean','arm_force_rl_mean','arm_force_rr_mean', ...
        'arm_force_fl_peak','arm_force_fr_peak','arm_force_rl_peak','arm_force_rr_peak','arm_force_peak_max','arm_force_imbalance', ...
        'impact_weight_n','impact_total_force_limit_n','impact_arm_imbalance_limit_n','impact_arm_peak_limit_n','contact_metrics_available', ...
        'final_state', ...
        'landing_cmd_time','pred_decision','executed_action','action_source','probe_episode','probe_reason','gt_safe_to_land','decision_outcome', ...
        'semantic_environment','semantic_drone_state','semantic_visual_state', ...
        'landing_feasibility', ...
        'wind_risk_enc','alignment_enc','visual_enc', ...
        'relation_wind_control','relation_visual_alignment','relation_estimation_conflict', ...
        'wind_body_force_x','wind_body_force_y','wind_body_force', ...
        'mean_wind_body_risk','mean_wind_gust_risk','mean_wind_dir_change_risk','mean_wind_risk_raw','wind_moment_risk','min_thrust_margin_n', ...
        'mean_estimation_uncertainty','gps_dropout_enabled','gps_dropout_start_sec','gps_dropout_duration_sec', ...
        'moving_pad_enabled','moving_pad_amp_u','moving_pad_amp_v','moving_pad_freq_hz', ...
        'trajectory_intent','trajectory_target_u','trajectory_target_v','trajectory_target_x','trajectory_target_y','trajectory_target_z','trajectory_quality', ...
        'navigation_instability','wind_hazardous_ratio', ...
        'scenario_policy','target_case', ...
        'launch_pid','launch_log','exception_message'
    };

    cols = intersect(wanted, tbl.Properties.VariableNames, 'stable');
    tbl = tbl(:, cols);

    % Backfill ontology-entity feature columns for model input consistency.
    cfg = autosimDefaultConfig();
    tbl = autosimEnsureOntologyFeatureColumns(tbl, cfg);
end


