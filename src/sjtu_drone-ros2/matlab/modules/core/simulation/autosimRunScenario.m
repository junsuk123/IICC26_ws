function [res, traceTbl] = autosimRunScenario(cfg, scenarioCfg, scenarioId, model, rosCtx)
    ...
    while true
        ...
        if analysisActive
            ...
            speedEvalNow = autosimNanLast(speedAbs(activeIdx));
            if ~isfinite(speedEvalNow)
                speedEvalNow = abs(vzEvalNow);
            end
            lateralSpeedNow = sqrt(max(0.0, speedEvalNow * speedEvalNow - vzEvalNow * vzEvalNow));
            speedTail = autosimTail(speedAbs(activeIdx), temporalHistN);
            vzTail = autosimTail(vz(activeIdx), temporalHistN);
            lateralSpeedHist = sqrt(max(0.0, speedTail .* speedTail - vzTail .* vzTail));
            droneObs = struct( ...
                ...
                'velocity', [lateralSpeedNow; 0.0; vzEvalNow], ...
                'lateral_speed', lateralSpeedNow, ...
                'lateral_speed_hist', lateralSpeedHist, ...
